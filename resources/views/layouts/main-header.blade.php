                                    ...
                                    <div id="unreadNotifications">
                                        @foreach (auth()->user()->unreadNotifications as $notification)
                                            <div class="main-notification-list Notification-scroll">
                                                <a class="d-flex p-3 border-bottom notification-link"
...
<script>
    document.querySelectorAll('.notification-link').forEach(link => {
        link.addEventListener('click', function (e) {
            e.preventDefault(); // منع التوجيه التلقائي
            const url = this.href; // حفظ الرابط الأساسي
            const notificationId = this.dataset.id; // الحصول على ID الإشعار

            // طلب Ajax لتحديث الإشعار كـ "مقروء"
            fetch(`/MarkAsRead/${notificationId}`, {
                ...
                .then(response => {
                    if (response.ok) {
                        // بعد تحديث الإشعار، انتقل إلى الرابط
